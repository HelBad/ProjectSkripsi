package com.example.projectskripsi.features.pesanan.data.source.remote

import android.util.Log
import com.example.projectskripsi.core.Response
import com.example.projectskripsi.features.pesanan.data.responses.KeranjangResponse
import com.example.projectskripsi.features.pesanan.data.responses.PesananResponse
    ...
    fun getPesanan(status: String, idUser: String?): Flowable<Response<ArrayList<PesananResponse>>> {
        ...
        ref.addValueEventListener(object : ValueEventListener {
                override fun onDataChange(p0: DataSnapshot) {
                    if (p0.exists()) {
                        val list = arrayListOf<PesananResponse>()
                        for (snap in p0.children) {
                            val menu = Converter.toObject(snap, PesananResponse::class.java)
